    ...
    def simulate(self):
        ...
        if self.algorithm == 'miso':
            ...
            for i in range(0, len(miso.GPU_list)):
                # miso_set_gi_id(miso.GPU_list[i], miso.config_list[i])
                miso_search_solution(miso.GPU_list[i], miso.config_list[i])
            ...
            while True:
                ...
                if len(job_list) == self.num and miso.online_job_queue.empty():
